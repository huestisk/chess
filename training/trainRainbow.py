...
from models.models import RainbowCnnDQN
...
class Rainbow(Trainer):
    ...
    def load_model(self):
        current_model = RainbowCnnDQN(self.env.observation_space.shape,
                                      self.env.action_space.n, num_atoms, Vmin, Vmax)  # input:(1,84,84), output:6
        target_model = RainbowCnnDQN(
            self.env.observation_space.shape, self.env.action_space.n, num_atoms, Vmin, Vmax)
